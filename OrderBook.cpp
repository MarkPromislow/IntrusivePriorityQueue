...
void OrderBook::initialize(Exchange * exchange, const char * symbol)
{
	_exchange = exchange;
...
bool OrderBook::newOrder(int side, unsigned price, BookOrder *order)
{
	...
	if (! _tradingMask)
	{
		...
		if (!inside(_price[otherSide], price))
		{
			...
			for (Intrusive::LinkedListObject *obj = levelList.begin(); obj != levelList.end(); obj = levelList.begin())
			{
...
void OrderBook::display()
{
	...
	printf("\t---\n");
	for (Intrusive::LinkedListObject *obj = buyLevels.begin(); obj != sellLevels.end(); obj = obj->next())
	{
		PriceLevel *priceLevel = static_cast<PriceLevel*>(obj);
		printf("\t%u %u %u\n", priceLevel->_price, priceLevel->_shares, priceLevel->_orderCnt);
	}
